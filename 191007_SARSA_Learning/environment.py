...
class Env(tk.Tk):
    ...
    def _build_canvas(self):
        ...
        self.rock1 = canvas.create_image(250, 150, image=self.shapes[1])
        self.rock2 = canvas.create_image(150, 250, image=self.shapes[1])
        self.meat = canvas.create_image(250, 250, image=self.shapes[2])
    ...
    def step(self, action):
        ...
        # move dog to top level of canvas
        self.canvas.tag_raise(self.dog)
        next_state = self.canvas.coords(self.dog)

        # reward function
        if next_state == self.canvas.coords(self.meat):
            reward = 100
            done = True
        elif next_state in [self.canvas.coords(self.rock1),
                            self.canvas.coords(self.rock2)]:
            reward = -100
            done = True
        else:
            reward = 0
